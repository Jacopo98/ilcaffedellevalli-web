...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  const [employeesResult, shiftsResult] = await Promise.all([
    context.supabase.from("employees").select("id, first_name, last_name, phone, email, role_title, hire_date, weekly_contract_hours, notes, color, is_active").order("is_active", { ascending: false }).order("first_name"),
