            ...
            <div className="mt-8 space-y-4">
              {category.menu_items.map((item) => (
                <details className="admin-item" key={item.id}>
                  <summary><strong>{item.name}</strong><span className="admin-item-meta"><small>{item.price_cents === null ? "Prezzo da definire" : (item.price_cents / 100).toLocaleString("it-IT", { style: "currency", currency: "EUR" })}</small><span className={item.is_available ? "text-ink/45" : "text-orange"}>{item.is_available ? "Disponibile" : "Nascosto"}</span></span></summary>
